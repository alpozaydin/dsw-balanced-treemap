...
public: // DO NOT CHANGE THIS PART.
    ...
    // for submap function in TreeMap.h
    void subMap(const T& fromElement, const T& toElement, std::vector<T>& result) const;
    void subMapHelper(Node<T>* node, const T& fromElement, const T& toElement, std::vector<T>& result) const;
...
template<class T>
void DSWTree<T>::rightRotation(Node<T>*& k2) {
    Node<T>* k1 = k2->left;
    k2->left = k1->right;
    k1->right = k2;
    // Update heights of k2 and k1:
    ...
    k2 = k1; // The reference now points directly to the rotated subtree root
}

template<class T>
void DSWTree<T>::leftRotation(Node<T>*& k2) {
    Node<T>* k1 = k2->right;
    k2->right = k1->left;
    k1->left = k2;
    // Update heights:
...
void DSWTree<T>::compress(Node<T>*& root, int m) {
    ...
    for (int i = 0; i < m; i++) {
        if ((*current) != NULL && (*current)->right != NULL) {
            // Rotate left at *current
            leftRotation(*current);
            // Move current one step to the right after rotation
            current = &((*current)->right);
        } else {
            break; // No more rotations possible
        }
    }
}
...
const T &DSWTree<T>::getFloor(const T &element) const {
    ...
    else // -> element is smaller than the smallest element in the tree
    {
        throw NoSuchItemException();
    }
}
...
// In DSWTree.cpp or DSWTree.hpp (if using templates)

template<class T>
void DSWTree<T>::subMap(const T& fromElement, const T& toElement, std::vector<T>& result) const {
    subMapHelper(root, fromElement, toElement, result);
}

template<class T>
void DSWTree<T>::subMapHelper(Node<T>* node, const T& fromElement, const T& toElement, std::vector<T>& result) const {
    if (!node) {
        return;
    }

    // Since T is KeyValuePair<K, V>, comparison operators should compare keys
    if (node->element > fromElement) {
        // Potentially keys in left subtree are in range
        subMapHelper(node->left, fromElement, toElement, result);
    }

    if (!(node->element < fromElement) && !(node->element > toElement)) {
        // node->element >= fromElement && node->element <= toElement
        result.push_back(node->element);
    }

    if (node->element < toElement) {
        // Potentially keys in right subtree are in range
        subMapHelper(node->right, fromElement, toElement, result);
    }
}
